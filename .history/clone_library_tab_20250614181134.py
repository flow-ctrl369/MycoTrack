...
class CloneLibraryTab(ttk.Frame):
    ...
    def setup_form(self):
        ...
        ttk.Button(button_frame, text="Archive Clone", command=self.archive_clone).pack(side=tk.LEFT, padx=5)
        ...
        try:
            ...
            if messagebox.askyesno("Confirm Archive", 
                ...
                cursor.execute('''
                    ...
                    WHERE clone_id = ?
                ''', (datetime.now().isoformat(), clone_id))
    ...
    def load_data(self):
        try:
            ...
            cursor.execute("SELECT * FROM clone_library WHERE archived = 0 ORDER BY date_taken DESC")
    ...
    def export_to_csv(self):
        try:
            filename = "clone_library_export.csv"
            
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # Get all data
            cursor.execute("SELECT * FROM clone_library WHERE archived = 0")
            rows = cursor.fetchall()
            
            # Get column names
            cursor.execute("PRAGMA table_info(clone_library)")
