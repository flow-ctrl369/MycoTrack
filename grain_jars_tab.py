...
class GrainJarsTab(ttk.Frame):
    ...
    def validate_jar_id(self, jar_id):
        ...
        if not jar_id:
            return False
        # Check if it matches the pattern GJ-XXXX where X is a digit
        return bool(re.match(r"^GJ-\d{4}$", jar_id))
    ...
    def setup_search(self):
        ...
        # Buttons frame for import/export
        button_frame = ttk.Frame(self.table_frame) # New frame for buttons
        button_frame.pack(fill=tk.X, pady=(0, 10)) # Pack it below the search bar

        ttk.Button(button_frame, text="Import from CSV", command=self.import_from_csv).pack(side=tk.RIGHT, padx=(5, 10), ipadx=8, ipady=4)
        ttk.Button(button_frame, text="Export to CSV", command=self.export_selected).pack(side=tk.RIGHT, padx=(0, 5), ipadx=8, ipady=4)
    ...
    def save_record(self):
        try:
            ...
            values = {
                ...
                "colonization_percentage": int(self.form_vars["colonization_percentage"].get()),
            ...
            if not self.validate_jar_id(values["jar_id"]):
                messagebox.showerror("Error", "Jar ID must be in format GJ-XXXX (e.g., GJ-0001)")
                return

            if not self.validate_source_id(values["source_id"]):
                messagebox.showerror("Error", "Source ID does not exist in Agar Plates or Liquid Cultures")
                return
    ...
    def edit_record(self, event):
        ...
        self.form_vars["colonization_percentage"].set(values[3])
        if values[4]: # Shake date might be empty
            self.form_vars["shake_date"].set_date(datetime.strptime(values[4], "%Y-%m-%d"))
        else:
            self.form_vars["shake_date"].set_date(None) # Clear date if empty
    ...
    def import_from_csv(self):
        ...
        try:
            with open(file_path, 'r', newline='') as csvfile:
                ...
                records_to_insert = []
                for row in reader:
                    # Assuming row order matches database schema (jar_id, source_id, inoculation_date, colonization_percentage, shake_date, contamination_notes)
                    # Handle potential empty shake_date
                    if len(row) > 4 and row[4] == '':
                        row[4] = None
                    records_to_insert.append(row + [self.db.get_timestamp(), self.db.get_timestamp()])

                self.db.import_records("grain_jars", records_to_insert)
                self.load_data()
                messagebox.showinfo("Success", f"{len(records_to_insert)} records imported successfully!")
