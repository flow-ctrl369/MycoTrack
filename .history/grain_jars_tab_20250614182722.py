...
class GrainJarsTab(ttk.Frame):
    ...
    def validate_jar_id(self, jar_id):
        ...
        if not jar_id:
            return False
        # Check if it matches the pattern GJ-XXX where X is a digit
        return bool(re.match(r'^GJ-\d{3}$', jar_id))
    ...
    def save_record(self):
        try:
            ...
            if not self.validate_jar_id(values["jar_id"]):
                messagebox.showerror("Error", "Jar ID must be in format GJ-XXX (e.g., GJ-001)")
                return
