...
class LiquidCultureTab(ttk.Frame):
    ...
    def setup_form(self):
        ...
        for i, (label_text, field_name) in enumerate(fields):
            ...
            if field_name == "date_created":
                ...
                self.form_vars[field_name] = ttk.Combobox(self.form_frame, values=["Untested", "Passed", "Contaminated"])
                self.form_vars[field_name].grid(row=i, column=1, sticky="w", padx=5, pady=2)
    ...
    def setup_table(self):
        # Create Treeview
        columns = ("LC ID", "Source ID", "Strain Name", "Date Created", 
    ...
    def save_record(self):
        try:
            ...
            values = {
                ...
                "date_created": self.form_vars["date_created"].get_date().isoformat(),
            ...
            cursor.execute('''
                ...
                (lc_id, source_id, strain_name, date_created, growth_description, 
    ...
    def load_data(self):
        try:
            ...
            cursor.execute("SELECT * FROM liquid_cultures ORDER BY date_created DESC")
    ...
    def edit_record(self, event):
        ...
        self.form_vars["date_created"].set_date(datetime.strptime(values[3], "%Y-%m-%d"))
    ...
    def export_to_csv(self):
        try:
            filename = "liquid_cultures_export.csv"
            
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            
            # Get all data
            cursor.execute("SELECT * FROM liquid_cultures")
            rows = cursor.fetchall()
            
            # Get column names
            cursor.execute("PRAGMA table_info(liquid_cultures)")
