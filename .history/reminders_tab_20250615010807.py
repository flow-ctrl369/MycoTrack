...
class RemindersTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # Notes Field
        ttk.Label(form_frame, text="Notes:").grid(row=7, column=0, padx=5, pady=2, sticky=tk.W)
        self.notes_entry = tk.Text(form_frame, width=50, height=3, wrap=tk.WORD) # Use Text widget for multi-line notes
        self.notes_entry.grid(row=7, column=1, padx=5, pady=2, sticky=tk.EW)

        # Category Field
        ttk.Label(form_frame, text="Category:").grid(row=9, column=0, padx=5, pady=2, sticky=tk.W)
        self.category_var = tk.StringVar(value="General")
        self.category_dropdown = ttk.Combobox(form_frame, textvariable=self.category_var,
                                               values=["General", "Cultivation", "Maintenance", "Harvest", "Other"],
                                               state="readonly", width=15)
        self.category_dropdown.grid(row=9, column=1, padx=5, pady=2, sticky=tk.W)

        # Email Notification Options
        self.send_email_var = tk.IntVar(value=0) # 0 for false, 1 for true
        self.send_email_checkbutton = ttk.Checkbutton(form_frame, text="Send Email Notification", variable=self.send_email_var)
        self.send_email_checkbutton.grid(row=11, column=0, columnspan=2, padx=5, pady=5, sticky=tk.W)

        ttk.Label(form_frame, text="Email Address (optional):").grid(row=12, column=0, padx=5, pady=2, sticky=tk.W)
        self.email_address_entry = ttk.Entry(form_frame, width=50)
        self.email_address_entry.grid(row=12, column=1, padx=5, pady=2, sticky=tk.EW)
        ...
        self.add_button = add_button # Store reference

        # New buttons for edit mode
        self.update_button = ttk.Button(form_frame, text="Save Changes", command=self.update_reminder)
        self.cancel_edit_button = ttk.Button(form_frame, text="Cancel Edit", command=self.cancel_edit)
        # Initially hide these buttons
        # self.update_button.grid(row=13, column=0, pady=10)
        # self.cancel_edit_button.grid(row=13, column=1, pady=10)
        ...
        self.reminders_tree = ttk.Treeview(list_frame, columns=("Task", "Date", "Time", "Priority", "Recurrence", "Notes", "Category"), show="headings")
        ...
        self.reminders_tree.heading("Notes", text="Notes")
        self.reminders_tree.heading("Category", text="Category")
        self.reminders_tree.column("Task", width=120)
        self.reminders_tree.column("Date", width=70, anchor=tk.CENTER)
        self.reminders_tree.column("Time", width=60, anchor=tk.CENTER)
        self.reminders_tree.column("Priority", width=60, anchor=tk.CENTER)
        self.reminders_tree.column("Recurrence", width=80, anchor=tk.CENTER)
        self.reminders_tree.column("Notes", width=150)
        self.reminders_tree.column("Category", width=80, anchor=tk.CENTER)
        ...
        # Initialize state to add mode
        self.current_edit_reminder_id = None # Stores the ID of the reminder being edited
        self.set_form_mode("add")

        # Bind right-click event to Treeview
        self.reminders_tree.bind("<Button-3>", self.show_context_menu) # Right-click
    ...
    def set_form_mode(self, mode):
        if mode == "add":
            ...
            self.current_edit_reminder_id = None
            self.clear_entries()
        elif mode == "edit":
    ...
    def add_reminder(self):
        ...
        # Validate email address if send_email is checked
        if send_email and not email_address:
            messagebox.showerror("Input Error", "Email address is required if 'Send Email Notification' is checked.")
            return
        # Basic email format validation (more robust validation can be added if needed)
        if send_email and "@" not in email_address or "." not in email_address:
            messagebox.showerror("Input Error", "Please enter a valid email address.")
            return

        self.db.cursor.execute("INSERT INTO reminders (task, reminder_date, reminder_time, completed, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes, category, send_email, email_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                (task, date_str, time_str, 0, recurrence_type, recurrence_interval, recurrence_end_date_str if recurrence_end_date_str else None, priority, notes if notes else None, category, send_email, email_address if email_address else None))
        self.db.conn.commit()
    ...
    def load_reminders(self):
        ...
        # Base query
        query = "SELECT id, task, reminder_date, reminder_time, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes, category, send_email, email_address FROM reminders WHERE completed = ?"
        ...
        for reminder in reminders:
            ...
            # Truncate notes for display in Treeview if they are too long
            notes_display = reminder[8] if reminder[8] else ""
            if len(notes_display) > 50:
                notes_display = notes_display[:47] + "..."

            self.reminders_tree.insert("", "end", iid=reminder[0], values=(reminder[1], reminder[2], reminder[3], reminder[7], recurrence_display, notes_display, reminder[9]))
        ...
        self.notes_entry.delete("1.0", tk.END) # Clear text from Text widget
    ...
    def update_reminder(self):
        if self.current_edit_reminder_id is None:
            messagebox.showerror("Error", "No reminder selected for update.")
            return
        ...
        self.db.cursor.execute("""
            UPDATE reminders SET
                ...
                email_address = ?
            WHERE id = ?
        """, (task, date_str, time_str, recurrence_type, recurrence_interval, 
             recurrence_end_date_str if recurrence_end_date_str else None, 
             priority, notes if notes else None, category, send_email, 
             email_address if email_address else None, self.current_edit_reminder_id))
        self.db.conn.commit()
        messagebox.showinfo("Success", "Reminder updated successfully!")
        self.set_form_mode("add") # Switch back to add mode
        self.load_reminders()
    ...
    def show_context_menu(self, event):
        # Select item closest to the click
        item = self.reminders_tree.identify_row(event.y)
        if item:
            self.reminders_tree.selection_set(item) # Select the clicked item
            self.reminders_tree.focus(item) # Set focus to the clicked item
            
            context_menu = tk.Menu(self.reminders_tree, tearoff=0)
            context_menu.add_command(label="Edit Reminder", command=self.edit_selected_reminder)
            try:
    ...
    def edit_selected_reminder(self):
        ...
        # Fetch all details for the selected reminder
        reminder_id = selected_item
        reminder_data = self.db.cursor.execute("SELECT task, reminder_date, reminder_time, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes, category, send_email, email_address FROM reminders WHERE id = ?", (reminder_id,)).fetchone()

        if reminder_data:
            # Unpack the data
            task, reminder_date, reminder_time, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes, category, send_email, email_address = reminder_data
            
            # Populate the form fields
            self.clear_entries() # Clear current form first
            self.task_entry.insert(0, task)
            self.date_entry.insert(0, reminder_date)
            self.time_entry.insert(0, reminder_time)
            self.recurrence_type_var.set(recurrence_type)
            self.recurrence_interval_entry.delete(0, tk.END)
            self.recurrence_interval_entry.insert(0, str(recurrence_interval))
            self.recurrence_end_date_entry.delete(0, tk.END)
            if recurrence_end_date: # Only insert if not None
                self.recurrence_end_date_entry.insert(0, recurrence_end_date)
            self.priority_var.set(priority)
            self.notes_entry.delete("1.0", tk.END)
            if notes: # Only insert if not None
                self.notes_entry.insert("1.0", notes)
            self.category_var.set(category)
            self.send_email_var.set(send_email)
            self.email_address_entry.delete(0, tk.END)
            if email_address: # Only insert if not None
                self.email_address_entry.insert(0, email_address)
            
            self.current_edit_reminder_id = reminder_id
            self.set_form_mode("edit")
        else:
            messagebox.showerror("Error", "Could not retrieve reminder data.") 