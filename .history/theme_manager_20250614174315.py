...
class ThemeManager:
    ...
    def update_widget_colors(self, widget, colors):
        ...
        try:
            ...
            # Handle specific Tkinter widgets that don't respond to ttk.Style
                ...
                widget.configure(bg=colors['bg'])
            elif isinstance(widget, tk.LabelFrame): # Explicitly handle tk.LabelFrame
                widget.configure(bg=colors['labelframe_bg'], fg=colors['labelframe_fg'])

            # Handle problematic ttk widgets that might not dynamically update via style.configure alone
            elif isinstance(widget, ttk.Frame): # Explicitly configure ttk.Frame backgrounds
                widget.configure(background=colors['bg'])
            elif isinstance(widget, ttk.LabelFrame): # Explicitly configure ttk.LabelFrame backgrounds
                widget.configure(background=colors['labelframe_bg'], foreground=colors['labelframe_fg'])
            elif isinstance(widget, ttk.Combobox):
                widget.configure(fieldbackground=colors['combobox_bg'], background=colors['combobox_bg'], foreground=colors['combobox_fg'], arrowcolor=colors['combobox_fg'])
            elif isinstance(widget, ttk.Spinbox):
                 widget.configure(fieldbackground=colors['spinbox_bg'], background=colors['spinbox_bg'], foreground=colors['spinbox_fg'], arrowcolor=colors['spinbox_fg'])
