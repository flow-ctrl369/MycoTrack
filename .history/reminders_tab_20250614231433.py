...
class RemindersTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        self.reminders_tree = ttk.Treeview(list_frame, columns=("Task", "Date", "Time", "Priority", "Recurrence", "Notes"), show="headings")
        ...
        self.reminders_tree.column("Task", width=150)
        self.reminders_tree.column("Date", width=80, anchor=tk.CENTER)
        self.reminders_tree.column("Time", width=70, anchor=tk.CENTER)
        self.reminders_tree.column("Priority", width=70, anchor=tk.CENTER)
        self.reminders_tree.column("Recurrence", width=90, anchor=tk.CENTER)
        self.reminders_tree.column("Notes", width=200)
    ...
    def add_reminder(self):
        ...
        notes = self.notes_entry.get("1.0", tk.END).strip() # Get text from Text widget
        ...
        self.db.cursor.execute("INSERT INTO reminders (task, reminder_date, reminder_time, completed, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                (task, date_str, time_str, 0, recurrence_type, recurrence_interval, recurrence_end_date_str if recurrence_end_date_str else None, priority, notes if notes else None))
        self.db.conn.commit()
    ...
    def load_reminders(self):
        ...
        # Base query
        query = "SELECT id, task, reminder_date, reminder_time, recurrence_type, recurrence_interval, recurrence_end_date, priority, notes FROM reminders WHERE completed = ?"
        ...
        for reminder in reminders:
            ...
            self.reminders_tree.insert("", "end", iid=reminder[0], values=(reminder[1], reminder[2], reminder[3], reminder[7], recurrence_display, notes_display))
