...
class MycoTrackerApp:
    ...
    def send_email_notification(self, recipient_email, task, reminder_date, reminder_time):
        # Get credentials from environment variables for security
        sender_email = os.getenv("MYCOTRACKER_EMAIL_SENDER_EMAIL")
        sender_password = os.getenv("MYCOTRACKER_EMAIL_SENDER_PASSWORD")
        smtp_server = os.getenv("MYCOTRACKER_EMAIL_SMTP_SERVER")
        smtp_port_str = os.getenv("MYCOTRACKER_EMAIL_SMTP_PORT")

        if not all([sender_email, sender_password, smtp_server, smtp_port_str]):
            messagebox.showerror("Email Error", "Email credentials not set as environment variables. Please set MYCOTRACKER_EMAIL_SENDER_EMAIL, MYCOTRACKER_EMAIL_SENDER_PASSWORD, MYCOTRACKER_EMAIL_SMTP_SERVER, and MYCOTRACKER_EMAIL_SMTP_PORT.")
            print("Email credentials not set. Email notification aborted.")
            return

        try:
            smtp_port = int(smtp_port_str)
        except ValueError:
            messagebox.showerror("Email Error", "SMTP Port environment variable must be a number.")
            print("SMTP Port environment variable is not a valid number. Email notification aborted.")
            return

        subject = f"MycoTracker Reminder: {task}"
        body = f"Hello,\n\nThis is a reminder for your task: {task}\nDue Date: {reminder_date}\nDue Time: {reminder_time}\n\nBest regards,\nMycoTracker App"
    ...
    def check_for_reminders(self):
        ...
        reminders_to_check = self.db.cursor.execute('''
            SELECT id, task, reminder_date, reminder_time, recurrence_type, recurrence_interval, recurrence_end_date, send_email, email_address
            FROM reminders
        ...
        for reminder_id, task, reminder_date_str, reminder_time_str, recurrence_type, recurrence_interval, recurrence_end_date_str, send_email, email_address in reminders_to_check:
            ...
            if send_email == 1 and email_address:
                self.send_email_notification(email_address, task, reminder_date_str, reminder_time_str)
