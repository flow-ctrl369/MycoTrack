...
class MycoTrackerApp:
    ...
    def check_for_reminders(self):
        ...
        reminders = self.db.cursor.execute('''
            SELECT id, task, reminder_date, reminder_time 
            FROM reminders 
            WHERE completed = 0 AND notified = 0 AND 
            (reminder_date < ? OR (reminder_date = ? AND reminder_time <= ?))
            ORDER BY reminder_date, reminder_time
        ''', (current_date, current_date, current_time)).fetchall()

        for reminder_id, task, reminder_date, reminder_time in reminders:
            messagebox.showinfo("Reminder", f"Reminder: {task} at {reminder_time} on {reminder_date}")
            self.db.cursor.execute("UPDATE reminders SET notified = 1 WHERE id = ?", (reminder_id,))
            self.db.conn.commit()
